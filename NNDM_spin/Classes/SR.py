...
class SR(object):
    ...
    def __init__(self, nqs, lindbladian, mh, io, path, learning_rate,params_sd, comm_world, method, lru_size, drawn, gpu):
        ...
        if method == 'ldag_full' or method == 'ldag_y':
            self.multiplier = -1;
        else:
            self.multiplier = +1;
    ...
    def sd_run(self, batch_nbr, oper):
        ...
        for batch in range(batch_nbr):
            ...
            pars = self.comm.bcast(pars, root=0);
...
#                 self.nqs.lookup_init(self.mh.S_i, self.mh.S_j);
            ...
            lru_usage = self.comm.reduce(sampled[7], op = oper, root = 0);
            ...
            if self.myrank == 0:
                ...
                lru_usage /= self.comm.size;

                if self.gpu:
                    with cp.cuda.Device(self.myrank):
                        params_update = self.stochastic_gradient(Lrho, Ok, Okk, LOk, batch); 
                else:
                    params_update = self.stochastic_gradient(Lrho, Ok, Okk, LOk, batch); 
                ...
                print('Estimated <rho|L|rho> after batch nbr {}:    {}  +/-   {}, move {}, total {}. '.format(batch+self.nqs.it_start, Lrho, L_sq, move_rate, total_sweep));
                
                self.io.Output_Param(self.nqs);
                self.io.Output_Estimator(batch+self.nqs.it_start, Lrho, L_sq);
                self.io.Output_Profiling(move_rate,lru_usage, MH_stop-MH_start, comm_done-MH_stop, SR_solve_done-comm_done);
          
            
    # Calculate new network parameters via SR
    def stochastic_gradient(self, lrho, Ok, Okk, LOk, it):
    
        sol = lsqr(Okk, LOk, atol=1e-11, btol=1e-11);
        return np.real(sol[0].reshape(self.nqs.paramNbr,1))
...
#         else:
#             sol = MinresQLP_cpu(S_kk_reg, F_k, rtol, maxit);   
#               
#         return np.real(sol[0]), np.linalg.cond(Okk)        
 

    # lambda regularization from paper
    @staticmethod
    def lambd(it, lambd0=100, b=0.98, lambdMin=1e-2, it_start=0):
