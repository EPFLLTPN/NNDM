...
def MinresQLP_gpu(A, b, rtol, maxit, M=None, shift=None, maxxnorm=None,
...
#     A = aslinearoperator(A)
    ...
    if show:
        ...
        print('Min-length solution of symmetric(singular)')
    ...
    #final quantities
    r1 = b - Ax(A,cp.array(x)) + shift*x
    rnorm = norm(r1)
    Arnorm = norm(Ax(A,cp.array(r1)) - shift*r1)
    xnorm = norm(x)
    relres = rnorm/(Anorm*xnorm + beta1)
    relAres = 0
...
#             r3 = r3 - r1*beta/betal
#         
#         alfa = cp.real(cp.dot(r3.T,v))
#         r3 = r3 - r2*alfa/beta
#         r1 = r2
#         r2 = r3
#         
#         if noprecon:
#             betan = cp.linalg.norm(r3)
#             if iters == 1:
#                 if betan == 0:
#                     if alfa == 0:
#                         flag = 0
...
#             else:
#                 print('Error: "M" is indefinite or singular!')
#         pnorm = cp.sqrt(betal ** 2 + alfa ** 2 + betan ** 2)
#         
#         #previous left rotation Q_{k-1}
#         dbar = dltan
#         dlta = cs*dbar + sn*alfa
...
#         if iters > 1:
#             ul = (taul - etal*ul3 - veplnl *ul2)/gamal
#         xnorm_tmp = cp.sqrt(xl2norm**2 + ul2**2 + ul**2)
#         if abs(gama) > cp.finfo(cp.double).tiny and xnorm_tmp < maxxnorm:
#             u = (tau - eta*ul2 - vepln*ul)/gama
#             if cp.sqrt(xnorm_tmp**2 + u**2) > maxxnorm:
#                 u = 0
#                 flag = 6
#         else:
#             u = 0
#             flag = 9
#         xl2norm = cp.sqrt(xl2norm**2 + ul2**2)
#         xnorm = cp.sqrt(xl2norm**2 + ul**2 + u**2)
#         #update w&x
#         #Minres
#         if (Acond < TranCond) and flag != flag0 and QLPiter == 0:
#             wl2 = wl
...
#             rnorm = phi
#         relres = rnorm / (Anorm * xnorm + beta1)
#         rootl = cp.sqrt(gbar ** 2 + dltan ** 2)
#         Arnorml = rnorml * rootl
#         relAresl = rootl / Anorm
#         ## See if any of the stopping criteria are satisfied.
#         epsx = Anorm * xnorm * cp.finfo(float).eps
#         if (flag == flag0) or (flag == 9):
#             t1 = 1 + relres
#             t2 = 1 + relAresl
#             if iters >= maxit:
...
#         else:
#             if rnormvec:
#                 resvec = cp.append(resvec, rnorm)
#                 Aresvec = cp.append(Aresvec, Arnorml)
#                 
#             if show:
#                 if iters%10 - 1 == 0:
#                     lstr = ('        iter     rnorm    Arnorm    relres   ' +
...
#     #final quantities
#     r1 = b - Ax(A,x) + shift*x
#     rnorm = cp.linalg.norm(r1)
#     Arnorm = cp.linalg.norm(Ax(A,r1) - shift*r1)
#     xnorm = cp.linalg.norm(x)
#     relres = rnorm/(Anorm*xnorm + beta1)
#     relAres = 0
#     if rnorm > cp.finfo(cp.double).tiny:
#         relAres = Arnorm/(Anorm*rnorm)    
#         
#     if show:
#         if rnorm > cp.finfo(cp.double).tiny:                
...
#         
#     if rnormvec:
#         Aresvec = cp.append(Aresvec, Arnorm)
#         return (x,flag,iters,Miter,QLPiter,relres,relAres,Anorm,Acond,
#                 xnorm,Axnorm,resvec,Aresvec)
#     
#     return (x,flag,iters,Miter,QLPiter,relres,relAres,Anorm,Acond,xnorm,Axnorm)
...
#     elif abs(b) > abs(a):
#         t = a / b
#         s = cp.sign(b) / cp.sqrt(1 + t ** 2)
#         c = s * t
#         r = b / s
#     else:
#         t = b / a
#         c = cp.sign(a) / cp.sqrt(1 + t ** 2)
#         s = c * t
#         r = a / c
#     return c, s, r
